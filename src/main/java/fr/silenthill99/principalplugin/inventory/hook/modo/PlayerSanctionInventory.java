...
public class PlayerSanctionInventory extends AbstractInventory<PlayerSanctionHolder> {
    ...
    public void manageInventory(InventoryClickEvent e, ItemStack current, Player player, PlayerSanctionHolder holder) {
        ...
        switch (current.getType()) {
            ...
            case SUNFLOWER -> InventoryManager.openInventory(player, InventoryType.MODO_PLAYER_MENU, target);
            case GREEN_DYE -> openInventory(player, target, type, page + 1);
            case RED_DYE -> openInventory(player, target, type, page - 1);
            case PINK_WOOL -> {
